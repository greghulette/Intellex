# =============================================================================
#  install-windows.ps1 - put NaviLink in the Start Menu
#
#  Copies dist\NaviLink.exe to %LOCALAPPDATA%\Programs\NaviLink and creates a
...
#  Start Menu entry, and pinning is one manual step: find NaviLink in the Start
#  Menu, right-click, "Pin to taskbar". Once.
# =============================================================================
$ErrorActionPreference = 'Stop'

$repo = Split-Path -Parent $PSScriptRoot
$exe  = Join-Path $repo 'dist\NaviLink.exe'
if (-not (Test-Path $exe)) {
    Write-Error "dist\NaviLink.exe not found. Run scripts\build-windows.bat first."
}

$dest = Join-Path $env:LOCALAPPDATA 'Programs\NaviLink'
...
Get-Process -Name 'NaviLink' -ErrorAction SilentlyContinue | ForEach-Object {
    Write-Host "Closing the running NaviLink (pid $($_.Id))..."
    $_.CloseMainWindow() | Out-Null
    Start-Sleep -Seconds 2
    if (-not $_.HasExited) { $_ | Stop-Process -Force }
}

Copy-Item $exe (Join-Path $dest 'NaviLink.exe') -Force
Write-Host "Installed to $dest"

$startMenu = Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs'
$lnk       = Join-Path $startMenu 'NaviLink.lnk'
$shell     = New-Object -ComObject WScript.Shell
$s         = $shell.CreateShortcut($lnk)
$s.TargetPath       = Join-Path $dest 'NaviLink.exe'
$s.WorkingDirectory = $dest
$s.IconLocation     = Join-Path $dest 'NaviLink.exe'
$s.Description      = 'Configure a NaviCore droid and its WCBs'
$s.Save()

Write-Host "Start Menu shortcut created."
Write-Host ""
Write-Host "To pin it: open the Start Menu, find NaviLink, right-click -> Pin to taskbar."
