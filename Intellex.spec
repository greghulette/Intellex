...
hiddenimports = ['fetch_webui', 'fetch_firmware', 'fetch_wiki', 'build_stamp']
...
for _d in ('webui', 'webui_wcb', 'firmware', 'wiki'):
...
for _f in ('launcher.html', 'shell.html', 'wiki.html', 'intellex_shim.js'):
    datas.append((os.path.join('src', _f), '.'))
datas.append((os.path.join('src', 'assets'), 'assets'))

# markdown-it-py renders the wikis at request time. wikidocs imports it inside a
# function -- deliberately, so a build with no docs never pays for it -- which is
# exactly the shape PyInstaller's static analysis cannot see.
for _pkg in ('esptool', 'serial', 'markdown_it'):
